...
public class SaslImpl implements Sasl, SaslFrameBody.SaslFrameBodyHandler<Void>,
    ...
    SaslImpl(TransportImpl transport, int maxFrameSize) {
        ...
        _frameWriter = new FrameWriter(_encoder, maxFrameSize,
    ...
    private void writeSaslOutput() {
        ...
        if (_logger.isLoggable(Level.FINER)) {
            _logger.log(Level.FINER,
                "Finished writing SASL output. Output Buffer : " +
                _outputBuffer);
        }
    }
    ...
    private void process() {
        ...
        if (_role == Role.SERVER) {
            ...
            if ((getState() == SaslState.PN_SASL_STEP) &&
            ...
            if (_done) {
                org.apache.qpid.proton.amqp.security.SaslOutcome outcome = new org.apache.qpid.proton.amqp.security.SaslOutcome();
                outcome.setCode(SaslCode.values()[_outcome.getCode()]);
                writeFrame(outcome);
            }
        } else if (_role == Role.CLIENT) {
            if ((getState() == SaslState.PN_SASL_IDLE) &&
                    (_chosenMechanism != null)) {
                processInit();
                _state = SaslState.PN_SASL_STEP;
    ...
    public String[] getRemoteMechanisms() {
        if (_role == Role.SERVER) {
            return (_chosenMechanism == null) ? new String[0]
                                              : new String[] {
    ...
    @Override
    public void handleResponse(SaslResponse saslResponse, Binary payload,
        Void context) {
        checkRole(Role.SERVER);
    ...
    private void checkRole(Role role) {
        if (role != _role) {
            throw new IllegalStateException("Role is " + _role +
                " but should be " + role);
        }
    }

    @Override
    public void handleMechanisms(SaslMechanisms saslMechanisms, Binary payload,
        Void context) {
        if (_role == null) {
            client();
        }
    ...
    public void handleOutcome(
        org.apache.qpid.proton.amqp.security.SaslOutcome saslOutcome,
        Binary payload, Void context) {
        checkRole(Role.CLIENT);
    ...
    public TransportWrapper wrap(final TransportInput input,
        final TransportOutput output) {
    ...
    public String toString() {
        ...
        builder.append("SaslImpl [_outcome=").append(_outcome).append(", state=")
               .append(_state).append(", done=").append(_done).append(", role=")
               .append(_role).append("]");
    ...
    private class SaslTransportWrapper implements TransportWrapper {
        ...
        private SaslTransportWrapper(TransportInput input,
        ...
        private boolean isOutputInSaslMode() {
            return (_role == null) ||
            ((_role == Role.CLIENT) && (!_done || !_initSent)) ||
        ...
        private void reallyProcessInput() throws TransportException {
            if (isInputInSaslMode()) {
                if (_logger.isLoggable(Level.FINER)) {
                    _logger.log(Level.FINER,
                        SaslImpl.this + " about to call input.");
                }
            ...
            if (!isInputInSaslMode()) {
                if (_logger.isLoggable(Level.FINER)) {
                    _logger.log(Level.FINER,
                        SaslImpl.this + " about to call plain input");
                }

...
import org.apache.qpid.proton.engine.impl.WebSocketTools;
...
public class SimpleSslTransportWrapper implements SslTransportWrapper
{
    ...
    private void unwrapInput() throws SSLException
    {
        while (true) {
            ...
            if (capacity == Transport.END_OF_STREAM) {
                ...
                try
                {
                    WebSocketTools.printBuffer("SimpleSslTransportWrapper received and decoded", _decodedInputBuffer);
                } catch (IOException e)
                {
                    e.printStackTrace();
                }
    ...
    private void wrapOutput() throws SSLException
    {
        while (true) {
            ...
            try
            {
                WebSocketTools.printBuffer("SimpleSslTransportWrapper is sending", clearOutputBuffer);
            } catch (IOException e)
            {
                e.printStackTrace();
            }
